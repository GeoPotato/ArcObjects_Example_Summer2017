...
Public Class frm_tornadoOptions
    ...
    Dim tornado_siteURL As String = "http://www.spc.noaa.gov/wcm/data/"
    ...
    Dim torn_headers As String = "om" & "," & "yr" & "," & "mo" & "," & "dy" & "," & "date" & "," & "time" & "," & "tz" & "," & "st" & "," & "stf" & "," &
                                "stn" & "," & "mag" & "," & "inj" & "," & "fat" & "," & "loss" & "," & "closs" & "," & "slat" & "," & "slon" & "," &
                                "elat" & "," & "elon" & "," & "len" & "," & "wid" & "," & "ns" & "," & "sn" & "," & "sg" & "," & "f1" & "," & "f2" & "," &
                                "f3" & "," & "f4" & "," & "fc"

    Private Sub frm_tornadoOptions_Load(sender As Object, e As EventArgs) Handles MyBase.Load

        'This function defines what will be populated within the ComboBox list for the tornado timeframes.

        Try

            lbl_torn_message.Visible = False

            txt_torn_url.Text = "URL will populate once Timespan has been selected."
            ...
            cb_torn_options_timespan.Items.Add("Select...")
            cb_torn_options_timespan.Items.Add("All | 1950-" & time_period.ToString)

            Do While (time_period >= 2008)

                cb_torn_options_timespan.Items.Add(time_period.ToString)
                time_period -= 1
            ...
            For Each pre2008 As String In timespan_pre2008
                cb_torn_options_timespan.Items.Add(pre2008.ToString)
            Next

            cb_torn_options_timespan.SelectedIndex = 0

        Catch ex As Exception

            MessageBox.Show("Error Location: frm_tornadoOptions_Load()" & vbCrLf & vbCrLf &
    ...
    Private Sub cb_torn_options_timespan_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cb_torn_options_timespan.SelectedIndexChanged

        Try

            Dim selectTime As String = cb_torn_options_timespan.SelectedItem
            ...
            Select Case selectTime
                ...
                Case = "All | 1950-" & time_period.ToString

                    timespan_url = "1950-" & time_period.ToString & "_torn"
                    txt_torn_url.Text = tornado_siteURL & timespan_url & fileExtCSV & fileExtZIP

                Case = cb_torn_options_timespan.SelectedItem.ToString

                    timespan_url = cb_torn_options_timespan.SelectedItem.ToString & "_torn"
                    txt_torn_url.Text = tornado_siteURL & timespan_url & fileExtCSV

            End Select

        Catch ex As Exception

            MessageBox.Show("Error Location: cb_torn_options_timespan_SelectedIndexChanged()" & vbCrLf & vbCrLf &
                            ex.Message & vbCrLf & vbCrLf & ex.StackTrace & vbCrLf & vbCrLf & ex.GetType.ToString,
                            errorMessage_header)
            Exit Sub
    ...
    Private Sub btn_torn_options_browse_Click(sender As Object, e As EventArgs) Handles btn_torn_options_browse.Click
        ...
        Catch ex As Exception
            ...
            MessageBox.Show("Error Location: btn_torn_options_browse_Click()" & vbCrLf & vbCrLf &
                            ex.Message & vbCrLf & vbCrLf & ex.StackTrace & vbCrLf & vbCrLf & ex.GetType.ToString,
                            errorMessage_header)
            Exit Sub
        ...
        Finally
            ...
            txt_torn_outputFolderpath.Text = folderPath

        End Try

    End Sub

    Private Sub btn_torn_options_OK_Click(sender As Object, e As EventArgs) Handles btn_torn_options_OK.Click

        Try

            'If the user fails to select a timespan, show error.
            If cb_torn_options_timespan.SelectedItem = "Select..." Then
        ...
        Catch ex As Exception
            ...
            MessageBox.Show("Error Location: btn_torn_options_OK_Click()" & vbCrLf & vbCrLf &
                            ex.Message & vbCrLf & vbCrLf & ex.StackTrace & vbCrLf & vbCrLf & ex.GetType.ToString,
                            errorMessage_header)
    ...
    Private Sub btn_torn_options_Cancel_Click(sender As Object, e As EventArgs) Handles btn_torn_options_Cancel.Click

        Try

            'Closes the window.

            frm_tornadoOptions.ActiveForm.Close()

        Catch ex As Exception

            MessageBox.Show("Error Location: btn_torn_options_Cancel_Click()" & vbCrLf & vbCrLf &
                            ex.Message & vbCrLf & vbCrLf & ex.StackTrace & vbCrLf & vbCrLf & ex.GetType.ToString,
                            errorMessage_header)
        ...
        Catch ex As Exception
            ...
            ElseIf countFiles > 1 Then
                ...
                MessageBox.Show("Oops! " & countFiles.ToString & " shapefile locks observed within folder:" & vbCrLf & folderPath & "\" &
                                noaa & timespan_url.Replace("-", "_to_") & curDate & vbCrLf & vbCrLf &
    ...
    Private Sub download_CSV()
        ...
        Try
            ...
            If timespan_url = "1950-" & time_period.ToString & "_torn" Then

                My.Computer.Network.DownloadFile(tornado_siteURL & timespan_url & fileExtCSV & fileExtZIP,
            ...
            Else
                ...
                My.Computer.Network.DownloadFile(tornado_siteURL & timespan_url & fileExtCSV,
                                                 folderPath & "\" & noaa & timespan_url.Replace("-", "_to_") & curDate & "\" &
                                                 noaa & timespan_url.Replace("-", "_to_") & curDate & fileExtCSV,
                                                 Nothing, Nothing, True, 100000, True)

                MessageBox.Show("Download complete for: " & vbCrLf & noaa & timespan_url.Replace("-", "_to_") & curDate & fileExtCSV &
                       vbCrLf & vbCrLf & "Extracting to:" & vbCrLf & folderPath & "\" & noaa & timespan_url.Replace("-", "_to_") & curDate)
        ...
        Try
            ...
            Do While SR.Peek <> -1
                ...
                If counter = 0 And strLineData(0).Contains("om") = False Then
                    ...
                    SWriter.WriteLine(torn_headers)
                ...
                If strLineData(15) <> "0.0" And strLineData(15) <> "0" And strLineData(16) <> "0.0" And strLineData(16) <> "0" Then
                    ...
                    SWriter.WriteLine(strLineData(0) & "," & strLineData(1) & "," & strLineData(2) & "," & strLineData(3) & "," & strLineData(4) & "," &
                                      ...
                                      strLineData(25) & "," & strLineData(26) & "," & strLineData(27) & "," & strLineData(28))
    ...
    Private Sub ifEmptyCSV()
        ...
        Try
            ...
            If File.ReadAllLines(folderPath & "\" & noaa & timespan_url.Replace("-", "_to_") & curDate & "\" &
                ...
                MessageBox.Show("No tornado features present within:" & vbCrLf &
                                noaa & timespan_url.Replace("-", "_to_") & curDate & "_checked" & fileExtCSV)
                Exit Sub
    ...
    Private Sub XYEvents()
        ...
        Try
            ...
            If checkbox_torn_addData.Checked = False Then

                'If Add to ArcMap checkbox is unchecked, exit.
                Exit Sub

            ElseIf checkbox_torn_addData.Checked = True Then
